...
public class Table
{
    ...
    private Collection _oFields = new Collection();
    private Collection _oFieldsKey = new Collection();
    ...
    ReflectionManager oReflection = new ReflectionManager();

    /// <summary>
    /// 
    /// </summary>
    /// <param name="oTable"></param>
    public Table(Table oTable)
    {
        _oData = oTable.DataManager;
        _strNameTable = oTable.TableName;
        _oFields = new Collection(oTable.Fields);
        _oFieldsKey = new Collection(oTable.FieldsKey);
        _oTypes = new Collection(oTable.Types);
    }
    /// <summary>
    /// 
    /// </summary>
    /// <param name="oDataManager"></param>
    public Table(DataManager oDataManager)
    {
        _oData = oDataManager;
    }
    /// <summary>
    ...
    public Table(DataManager oDataManager, string strNameTable, GCollection<string, DBField> oCCField, GCollection<string, DBField> oCCFieldKey)
    {
        _oData = oDataManager;
        ...
        {
            oAAField.Add(GetXMLToSQLType(((DBField)oCCField.GetValue(i)).Type));
        }
        ...
        {
            oAAFieldKey.Add(GetXMLToSQLType(((DBField)oCCFieldKey.GetValue(i)).Type));
        }
    ...
    private void SetProperties(ref object oObject, DataRow oDR, Collection oMatch)
    {
        try
        {
    ...
    public object[] ConvertDataTableToObjects(DataTable oDataTable, Type cType, string sFilter, string sSort)
    {
        ...
        for (int i = 0; ovDR != null && i < ovDR.Length; i++)
        {
            ...
            SetProperties(ref oB, ovDR[i], (Collection)_oTypes[cType.ToString()]);
            oArr.Add(oB);
        }
        return oArr.ToArray();
    }
    ...
    private object[] ObjectToArray(object oObject)
    {
        if (!this._oTypes.ContainsKey(oObject.GetType().ToString()))
            throw (new Exception("Type not supported"));
        Collection oMatch = (Collection)_oTypes[oObject.GetType().ToString()];
    ...
    public DataTable GetFilter(string sKeyConnection, params object[] oValues)
    {
        try
        {
            ...
            DataSet ds;
            string strSQL = "SELECT " + soFileds + " FROM " + _strNameTable + " WHERE ";
            Collection oFileds = GetFields(oValues);
        ...
        try
        {
            ...
            if (ds.Tables["Main"].Rows.Count > 0)
            {
                ...
                if (oValuesNoKey.Length == 0)
                    return false;
                Collection oFieldsNoKey = GetFieldsNoKey();
    ...
    private string _generateWHARE(Collection ovFileds, object[] oValoreCampi, string sSeparator)
    {
        string strSQL = "";
        //
    ...
    private string _generateMERGEFileds(Collection ovFileds, object[] oValoreCampi, string sSeparator)
    {
        string strSQL = "";
        //
        for (int i = 0; i < ovFileds.Count; i++)
    ...
    //
    private string _generateMERGEValues(Collection ovFileds, object[] oValoreCampi, string sSeparator)
    {
        string strSQL = "";
        //
    ...
    private string _generateAND(Collection ovFileds, object[] oValoreCampi)
    {
    ...
    {
        Collection oFileds = new Collection();
        for (int i = 0; i < oValoreCampi.Length; i++)
        {
    ...
    private Collection GetFieldsNoKey()
    {
    ...
    private Collection GetFields(object[] oValoreCampi)
    {
        Collection oCC = new Collection();
        for (int i = 0; i < _oFields.Count; i++)
        {
    ...
    private string _concat(Collection oFields, string sSymbol)
    {
        string strSQL = "";
...
/// </summary>
public class Table1N
{
    private string _strKey = null;
    private DataManager _oBus = null;
    private Table _oTParent = null;
    private Table _oTChild = null;
    private Collection _oRelation = null;
    /// <summary>
    /// 
    /// </summary>
    /// <param name="strKey"></param>
    /// <param name="oBus"></param>
    public Table1N(string strKey, DataManager oBus)
    {
        _strKey = strKey;
        _oBus = oBus;
    }
    ...
    public Table1N(Table oTParent, Table oTChild, Collection oRelation)
    {
        _oTParent = oTParent;
        _oTChild = oTChild;
    ...
    protected void Init(Table oTParent, Table oTChild, string sRelation)
    {
        _oTParent = oTParent;
        _oTChild = oTChild;
        _oRelation = new Collection();
        //
    ...
    public void Del(params object[] oValuesKey)
    {
        try
        {
            ...
            //
            Collection oCWhere = _oRelation;
            Collection oCWhere2 = new Collection();
            if (oCWhere.Count == oValuesKey.Length)
            {
                for (int i = 0; i < oCWhere.Count; i++)
                {
    ...
    private string _generateAND(Collection oRelation)
    {
        string strSQL = "";
        //
        int contNULL = 0;
    ...
    // Questa funzione costruisce una stringa pronta per la clausula SELECT
    // concatenando i campi tramite virgola
    private string _generateKeyVIRGULE(Collection oCampi)
    {
        string strSQL = "";
