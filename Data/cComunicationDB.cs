...
{
    cGCollection<string, string> getTables();
    cGCollection<string, cDBField> getFields(string strTable);
    cGCollection<string, cDBField> getFieldsKey(string strTable);
}
/// <summary>
/// 
/// </summary>
public class cDBField
{
    ...
    public cDBField(cDBField oField)
    {
        sName = oField.sName;
        sType = oField.sType;
...
{
    private cDataManager _oBUS;
    private string strConn;

    public SQLDBRead(string strKey, cDataManager oBUS)
    {
        _oBUS = oBUS;
        strConn = strKey;
    }
    public cGCollection<string, string> getTables()
    {
        DataSet ds;
        ds = _oBUS.GetDS(strConn, "Select name From sysobjects Where xtype = 'U' and name <> 'dtproperties' ORDER BY name", "Tables");
        cGCollection<string, string> oC = new cGCollection<string, string>();
        if (ds != null)
        {
    ...
    //
    public cGCollection<string, cDBField> getFields(string strTable)
    {
        cGCollection<string, cDBField> myStrColl = new cGCollection<string, cDBField>();
        DataSet ds;
        //
        ...
        {
            cDBField oDBF = (cDBField)oCC[Convert.ToString(ds.Tables["Fields"].DefaultView[i][0])];
    ...
    public StringDictionary getFieldsValue(string strTable)
    {
        ...
        cGCollection<string, cDBField> oFields;
        string myValue = "";
        string myType = "";
        //
        ...
        {
            cDBField oDBF = (cDBField)oFields.GetValue(i);
            myType = oDBF.Type;
            myValue = "[" + myType + "] ";
            myValue += "(" + oDBF.Length + ") ";
    ...
    public cGCollection<string, cDBField> getFieldsKey(string strTable)
    {
        cGCollection<string, cDBField> oCC = getFields(strTable);
        cGCollection<string, cDBField> oReturn = new cGCollection<string, cDBField>();
        for (int i = 0; i < oCC.Count; i++)
        {
            cDBField oDBF = (cDBField)oCC.GetValue(i);
            if (oDBF.PrimaryKey)
    ...
    public cCollection GetStoreProcedures()
    {
        cCollection myStrStore = new cCollection();
        DataSet ds, ds2;
...
public class SQLDBWrite
{
    ...
    //
    public SQLDBWrite(string strKey, cDataManager oData)
    {
        _oBUS = oData;
        strConn = strKey;
    ...
    //
    private cCollection Init(SQLDBRead oSQLRead, XMLDBRead oXMLRead)
    {
        cCollection oCCResult = new cCollection();
        cCollection oCCTablesDB = new cCollection();
        cCollection oCCTablesXML = new cCollection();
        //
    ...
    {
        cGCollection<string, cDBField> ColumsXML;
        cCollection ColumsKey;
        cCollection ColumsDefaultValue;
        //
        cCollection oCCTables = Init(oSQLRead, oXMLRead);
        //
        string sTableName = "";
        string sOperation = "";
        ...
        //
        cCollection oCStore = oXMLRead.getStoreProcedures();
    ...
    public bool CreateTable(string strTable, cGCollection<string, cDBField> oFields)
    {
        ...
        for (int i = 0; i < oFields.Count; i++)
        {
            ...
            if (virgola)
                mySQL += ", ";
            mySQL += " [" + mykey + "] ";
            mySQL += " " + ((cDBField)oFields[mykey]).GetStringColumn() + " ";
            virgola = true;
        }
        //
        mySQL += " ) ";
    ...
    public bool AlterTable(string strTable, cGCollection<string, cDBField> Colums)
    {
        //StringDictionary ColumsDB;
        //ColumsDB  = oSQLRead.getFieldsValue(strTable);
        cGCollection<string, cDBField> ColumsDB = oSQLRead.getFields(strTable);
        //
        ...
        for (int i = 0; i < Colums.Count; i++)
        {
            ...
            try
            {
                ...
                {
                    AlterColumnTable(strTable, (cDBField)Colums[mykey]);
                }
                else
                {
                    CreateColumnTable(strTable, (cDBField)Colums[mykey]);
                }
            }
            catch (Exception e)
            {
    ...
    public bool CreateColumnTable(string strTable, cDBField oField)
    {
        string mySQL = "ALTER TABLE " + strTable + " ";
        try
        {
    ...
    public bool AlterColumnTable(string strTable, cDBField oField)
    {
        string mySQL = "ALTER TABLE " + strTable + " ";
        try
        {
    ...
    {
        cCollection ostrDir = oSQLRead.GetStoreProcedures();
        //
        if (ostrDir == null)
            return false;
        //
    ...
    */
    ...
    public bool setPrimaryKey(string strTable, cCollection ColumsKey)
    {
        string mySQL = "";
        bool virgola = false;
        //
    ...
    public bool setDefaultValue(string strTable, cCollection Colums)
    {
        string mySQL = "";
        bool virgola = false;
        //
        cGCollection<string, cDBField> strcFields = oSQLRead.getFields(strTable);
        for (int i = 0; i < strcFields.Count; i++)
        {
            delDefaultValue(strTable, ((cDBField)strcFields.GetValue(i)).Name);
        }
        //
        mySQL += "ALTER TABLE " + strTable + " WITH NOCHECK ADD ";
        //
    ...
    public void delDefaultValue(string strTable, cCollection oCFields)
    {
...
public class XMLDBWrite : cXMLManager
{
    ...
    {
        cGCollection<string, cDBField> ColumsDB;
        cCollection ColumsKey;
        //
        cCollection oCCTables = Init(oSQLRead, oXMLRead);
        //
        ...
        cCollection oCStore = oSQLRead.GetStoreProcedures();
        for (int j = 0; j < oCStore.Count; j++)
        {
    ...
    //
    public void CreateColumn(string strTable, cDBField oField)
    {
    ...
    //
    public bool CreateTable(string strTable, cGCollection<string, cDBField> Columns)
    {

        for (int i = 0; i < Columns.Count; i++)
        {
            cDBField oF = Columns.GetValue(i);
            CreateColumn(strTable, oF);
        }

        return true;
    }
    public bool AlterTable(string strTable, cGCollection<string, cDBField> Columns)
    {
    ...
    //
    public bool setPrimaryKey(string strTable, cCollection oFields)
    {
        if (oFields == null)
            return false;
        delPrimaryKey(strTable);
        for (int i = 0; i < oFields.Count; i++)
        {
            if (GetX("/" + FIRSTNODEXML + "/" + TABLES + "/" + TABLE + "[@name='" + strTable + "']/" + COLUMN + "[@name='" + ((cDBField)oFields.GetValue(i)).Name + "']") != null)
                InsertX("/" + FIRSTNODEXML + "/" + TABLES + "/" + TABLE + "[@name='" + strTable + "']/" + COLUMN + "[@name='" + ((cDBField)oFields.GetValue(i)).Name + "']", "primarykey", "True");
        }
        return true;
    }
    //
...
/// </summary>
public class XMLDBRead : cXMLManager, IReader
{
    private string FIRSTNODEXML = "registry";
    string MAPS = "maps";
    string TYPE = "type";
    ...
    public cGCollection<string, string> getTables()
    {
        ...
        //
        cGCollection<string, string> oAlias = getAlias();
        cGCollection<string, string> oTables = new cGCollection<string, string>();
    ...
    /// </summary>
    /// <returns></returns>
    public cGCollection<string, string> getAlias()
    {
        cGCollection<string, string> ovAlias = new cGCollection<string, string>();
    ...
    /// <returns></returns>
    public cDBField getField(string sTableName, string strField)
    {
        string[] sNames = null;
    ...
    */
    ...
    /// <param name="strTable"></param>
    /// <returns></returns>
    public cCollection getFieldsPrimaryKey(string strTable)
    {
        cCollection oCC = new cCollection();
        string[] sValues = null;
    ...
    /// <param name="strTable"></param>
    /// <returns></returns>
    public cCollection getFieldsDefaultValues(string strTable)
    {
    ...
    /// <param name="strTable"></param>
    /// <returns></returns>
    public cCollection getFieldsNoDefaultValues(string strTable)
    {
        string[] oFields = GetX(FIRSTNODEXML + "/" + TABLES + "/" + TABLE + "[@name='" + strTable + "']/" + COLUMN + "[@default='']/@name");
        string[] oValues = GetX(FIRSTNODEXML + "/" + TABLES + "/" + TABLE + "[@name='" + strTable + "']/" + COLUMN + "[@default='']/@default");
        cCollection oCC = new cCollection();
