...
/// </summary>
public class cDataManager : MarshalByRefObject
{
    protected cProviders _oProviders = new cProviders();
    protected cConnections _oCns = null;
    protected cTransactions _oTrs = null;
    protected cGCollection<string, string> _Inclusions = new cGCollection<string, string>();
    ...
    public cDataManager()
    {
        lock (this)
        {
            try
            {
                _oCns = new cConnections(_oProviders);
                _oTrs = new cTransactions(_oCns);
            }
            catch (Exception e)
            {
                throw (e);
    ...
    /// </summary>
    public cTransactions Transactions
    {
        get { return _oTrs; }
    }
    /// <summary>
    ...
    /// </summary>
    public cGCollection<string, string> Inclusions
    {
    ...
    public cDataReader ExecuteReader(string sKeyConnection, string strSQL, string strParams)
    {
        lock (this)
        {
            cDataReader oDRE = null;

            cCommand oCMD = new cCommand(_oCns[sKeyConnection], strSQL);
            if (_iTimeoutCommand >= 0)
                oCMD.CommandTimeout = _iTimeoutCommand;
            oCMD.Parameters.AddParameterString(strParams);
            oDRE = new cDataReader(oCMD.ExecuteReader());

            return oDRE;
        }
    }

    public cDataReader ExecuteReader(cConnection oConnection, string strSQL, string strParams)
    {
        lock (this)
        {
            cDataReader oDRE = null;
            try
            {
                cCommand oCMD = new cCommand(oConnection, strSQL);
                if (_iTimeoutCommand >= 0)
                    oCMD.CommandTimeout = _iTimeoutCommand;
                oCMD.Parameters.AddParameterString(strParams);
                oDRE = new cDataReader(oCMD.ExecuteReader());
    ...
    public object ExecuteScalar(cConnection oConnection, string strSQL, string strParams)
    {
        lock (this)
        {
            object oObject = null;
    ...
    public int ExecuteNonQuery(cConnection oConnection, string strSQL, string strParams)
    {
        lock (this)
        {
            cCommand oCMD = new cCommand(oConnection, strSQL);
    ...
    /// <returns></returns>
    public cCommand GetCM(string sKeyConnection)
    {
        lock (this)
        {
            try
            {
                cCommand oCMD = new cCommand(_oCns[sKeyConnection]);
                if (_iTimeoutCommand >= 0)
                    oCMD.CommandTimeout = _iTimeoutCommand;
                return oCMD;
    ...
    /// <returns></returns>
    public cCommand GetCM(cConnection oConnection)
    {
        lock (this)
        {
            try
            {
                cCommand oCMD = new cCommand(oConnection);
                if (_iTimeoutCommand >= 0)
                    oCMD.CommandTimeout = _iTimeoutCommand;
                return oCMD;
    ...
    /// <param name="strSQL">Stringa SQL</param>
    /// <returns></returns>
    public cCommand GetCM(string sKeyConnection, string strSQL)
    {
        lock (this)
        {
            try
    ...
    /// <returns></returns>
    public cCommand GetCM(cConnection oConnection, string strSQL)
    {
        lock (this)
        {
            try
    ...
    /// <param name="sKeyConnection">Chiave di connessione</param>
    /// <param name="strSQL">stringa SQL</param>
    public cDataAdapter GetDA(string sKeyConnection, string strSQL)
    {
        lock (this)
        {
            try
            {
                return new cDataAdapter(GetCM(sKeyConnection, strSQL));
            }
            catch (Exception e)
            {
                throw (e);
    ...
    /// <param name="strSQL"></param>
    /// <returns></returns>
    public cDataAdapter GetDA(cConnection oConnection, string strSQL)
    {
        lock (this)
        {
            try
            {
                return new cDataAdapter(GetCM(oConnection, strSQL));
            }
            catch (Exception e)
            {
                throw (e);
    ...
    /// </summary>
    /// <param name="sKeyConnection">Chiave di connessione</param>
    public cDataAdapter GetDA(string sKeyConnection)
    {
        lock (this)
        {
            return new cDataAdapter(GetCM(sKeyConnection));
        }
    }

    public cDataAdapter GetDA(cConnection oConnection)
    {
        lock (this)
        {
            return new cDataAdapter(GetCM(oConnection));
        }
    }
    /// <summary>
    ...
    /// </summary>
    /// <param name="sKeyConnection">Chiave di connessione</param>
    public cConnection GetCO(string sKeyConnection)
    {
        lock (this)
        {
    ...
    public DataSet GetDS(cConnection oConnection, string strSQL)
    {
        lock (this)
        {
            DataSet odsTemp = new DataSet();
    ...
    /// <param name="strSQL"></param>
    /// <returns></returns>
    public DataTable GetDT(cConnection oConnection, string strSQL)
    {
        lock (this)
        {
    ...
    /// <returns></returns>
    public DataSet GetDS(cConnection oConnection, string strSQL, string MappingTableName)
    {
        lock (this)
        {
            DataSet odsTemp = new DataSet(MappingTableName);
    ...
    /// <returns></returns>
    public DataSet GetDS(cConnection oConnection, string strSQL, string MappingTableName, DataSet dsSource)
    {
        lock (this)
        {
            if (dsSource == null)
    ...
    public object InvokeStore(string sKeyConnection, string sNameStore, string[] ovNameFields, object[] ovValueFields)
    {
        lock (this)
        {
            ...
            cCommand oCM = new cCommand(_oCns[sKeyConnection], sNameStore);
            if (_iTimeoutCommand >= 0)
                oCM.CommandTimeout = _iTimeoutCommand;
            oCM.CommandType = CommandType.StoredProcedure;
            cParameters oPS = new cParameters(_oCns[sKeyConnection], oCM);
            for (int i = 0; i < ovNameFields.Length; i++)
            {
                oPS.Add(ovNameFields[i], ovValueFields[i]);
            }
            //
            cParameter oP = new cParameter(_oCns[sKeyConnection]);
    ...
    public object InvokeStore(cConnection oConnection, string sNameStore, string[] ovNameFields, object[] ovValueFields)
    {
        lock (this)
        {
            object oReturn = null;
            if (ovNameFields.Length != ovValueFields.Length)
                return null;
            cCommand oCM = new cCommand(oConnection, sNameStore);
            if (_iTimeoutCommand >= 0)
                oCM.CommandTimeout = _iTimeoutCommand;
            oCM.CommandType = CommandType.StoredProcedure;
            cParameters oPS = new cParameters(oConnection, oCM);
            for (int i = 0; i < ovNameFields.Length; i++)
            {
                oPS.Add(ovNameFields[i], ovValueFields[i]);
            }
            //
            cParameter oP = new cParameter(oConnection);
            oP.ParameterName = "Return_Value";
            oP.Direction = ParameterDirection.ReturnValue;
            oP.DbType = DbType.Object;
            oPS.Add(oP);
    ...
    public DataTable InvokeStore(string sKeyConnection, string sNameStore, params cParameter[] oParams)
    {
        lock (this)
        {
    ...
    public DataTable Invoke(cConnection oConnection, string sSQL, params cParameter[] oParams)
    {
        lock (this)
        {
            IDataReader oDataReader;
            ...
            cCommand oCM = new cCommand(oConnection, sSQL);
            if (_iTimeoutCommand >= 0)
                oCM.CommandTimeout = _iTimeoutCommand;
    ...
    public DataTable InvokeStore(cConnection oConnection, string sNameStore, params cParameter[] oParams)
    {
        lock (this)
        {
            IDataReader oDataReader;
    ...
    /// <returns></returns>
    public cParameter CreateParameter(string sKeyConnection, System.Data.DbType oDbType, System.Data.ParameterDirection oDirection, string sName, object oValue)
    {
        return new cParameter(Connections[sKeyConnection], oDbType, oDirection, sName, oValue);
    }
    /// <summary>
    /// 
    /// </summary>
    ...
    /// <param name="oValue"></param>
    /// <returns></returns>
    public cParameter CreateParameter(cConnection oConnection, System.Data.DbType oDbType, System.Data.ParameterDirection oDirection, string sName, object oValue)
    {
        return new cParameter(oConnection, oDbType, oDirection, sName, oValue);
    }
    ...
    {
        cXMLManager oRegSchema = new cXMLManager();
    ...
    public void Load(cXMLManager oXMLManager)
    {
        ...
        for (int i = 0; sProvidersKey != null && i < sProvidersKey.Length; i++)
        {
            ...
            //
            sAssembly[0] = cFileManager.GetPathRoot(cFileManager.NormalizePath(sAssembly[0]));
