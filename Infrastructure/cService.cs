...
{
    public cDBHttpClients DBHttpClients = null;
    public cDBConfig DBConfig = null;
    public cService(cDB oDB)
    {
    ...
    public string InvokeService(cCache<string> oCache, int minuteExpiration, string sCodice, Dictionary<string, string> oParams = null)
    {
        string s = oParams == null ? "" : string.Join(";", oParams.Select(x => x.Key + "=" + x.Value).ToArray());
        cCacheKey cacheKey = new cCacheKey(sCodice + "_" + s);
        string sResponse = oCache.get(cacheKey);
        if (sResponse == null)
        {
            sResponse = InvokeService(sCodice, oParams);
            cCacheValue<string> cacheValue = new cCacheValue<string>(sResponse, minuteExpiration, true);
    ...
    private string ReplaceDefine(string sValue)
    {
        ...
        {
            cString oVal = new cString(sValue);
