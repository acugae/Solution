...
public class DBQuery : DBEntity
{
    ...
    public DataTable GetQueryExecute(string sCode, GCollection<string, object> oParams = null, bool bOnlyVerified = false)
    {
        try
        {
            ...
            DataTable oDT = DB.Get(dbKey, sSQL);
    ...
    {
        return DB.Get(dbKey, "select syint_QueryLinks.* from syint_QueryLinks inner join syint_Query ON qu_id = ql_idQuery where qu_codice = '" + sCode + "'");
    }
    public DataTable GetQueryParams(string sCode)
    {
        return DB.Get(dbKey, "select syint_QueryParams.* from syint_QueryParams inner join syint_Query ON qu_id = qp_idQuery where qu_codice = '" + sCode + "'");
    }
    public int DelQuery(string sCode)
    {
        DataTable oDT = DB.Get(dbKey, "SELECT qu_id FROM [syint_Query] WHERE qu_codice = '" + sCode + "'");
        if (oDT != null && oDT.Rows.Count > 0)
        {
            DB.Execute(dbKey, "DELETE FROM [syint_QueryParams] WHERE qp_idQuery = " + oDT.Rows[0]["qu_id"].ToString());
            DB.Execute(dbKey, "DELETE FROM [syint_QueryLinks]  WHERE ql_idQuery = " + oDT.Rows[0]["qu_id"].ToString());
            return DB.Execute(dbKey, "DELETE FROM [syint_Query] WHERE qu_id = " + oDT.Rows[0]["qu_id"].ToString());
        }
        return 0;
    }
    public int DelQueryParams(string sCode, string sName)
    {
        ...
        if (oDT != null && oDT.Rows.Count > 0)
        {
            return DB.Execute(dbKey, "DELETE FROM [syint_QueryParams] WHERE qp_idQuery = " + oDT.Rows[0]["qu_id"].ToString() + " AND qp_name = '" + sName + "'");
        }
        return 0;
    }
    ...
    public int SetQuery(string sCode, string sName, string sScript, string sKeyDB, int iType, int iActive, string sOnLoad)
    {
        ...
        DataTable oDT = DB.Get(dbKey, "SELECT qu_codice FROM syint_Query WHERE qu_codice = '" + sCode + "'");
        if (oDT == null || oDT.Rows.Count == 0)
        {
    ...
    public int SetQueryLink(string sCode, string sName, string sType, string sUrl, string sParams, int iOrder, int iVisible, int iChecked)
    {
        ...
        //
        DataTable oDTLinks = DB.Get(dbKey, "SELECT * FROM [syint_QueryLinks] WHERE ql_idQuery = " + oDT.Rows[0]["qu_id"].ToString() + " AND ql_name = '" + sName + "'");
    ...
    public int SetQueryParams(string sCode, string sName, string sDescri, int iOrder, string sType, string sControl, string sSource, int iAllowNull, string sReplace)
    {
        ...
        if (oDT == null || oDT.Rows.Count == 0)
            throw new Exception("Query non trovata");
        //
        DataTable oDTParams = DB.Get(dbKey, "SELECT * FROM [syint_QueryParams] WHERE qp_idQuery = " + oDT.Rows[0]["qu_id"].ToString() + " AND qp_name = '" + sName + "'");
        DataRow oDR = oDT.Rows[0];
