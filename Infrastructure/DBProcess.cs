...
public class DBProcess : DBEntity
{
    ...
    {
        CRUD oCrud = new(DB, dbKey);
        CRUDBase tabService = new(entityName);
        tabService["pr_id"] = ID;
    ...
    public DataRow ReadProcess(Guid ID)
    {
        try
        {
            return DB.Get(dbKey, "SELECT * FROM core_Processes WHERE pr_id = '" + ID.ToString() + "'").Rows[0];
        }
        catch { return null; }
    }
    ...
    {
        CRUD oCrud = new(DB, dbKey);
        CRUDUpdate tabService = new(entityName);
